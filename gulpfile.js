...
gulp.task('build.dist', function () {
    return gulp.src(bowerConfig.publicDir)
        .pipe(gulpWebpack(webpackConfig))
        .pipe(gulp.dest(__dirname + '/../' + bowerConfig.name + '-build/' + bowerConfig.publicDir));
});

gulp.task('build.htmlCopy', function () {
    return gulp.src('./public/*.html')
        .pipe(gulp.dest(__dirname + '/../' + bowerConfig.name + '-build/public'));
});
...
function getBuildTasks () {
    return [
        'build.test',
        ['build.dist', 'build.htmlCopy', 'build.bump']
    ];
}
...
gulp.task('version', function () {
    isVersionTask = true;
    var tasks = [].concat(getBuildTasks(), getGitTasks());
